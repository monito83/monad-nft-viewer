...
function getEstadoClass(e) {
  return /abierto/i.test(e) ? 'Abierto'
       : /cerrado/i.test(e) ? 'Cerrado'
       : /pendiente/i.test(e) ? 'Pendiente'
       : 'Otro';
}
function parseImgTags(text) {
  if (!text) return '';
  return text.replace(/\[img\](https?:\/\/\S+)/g, (_,url) =>
...
Papa.parse(SHEET_CSV, {
  ...
  complete: results => {
    ...
    document.getElementById('faqBtn').onclick = () => {
      ...
      document.getElementById('faqContent').innerText = faqEntry ? faqEntry['FAQ'] : 'Sin FAQ configurada.';
